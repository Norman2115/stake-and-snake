...
export function handleTGameEnded(event: TGameEndedEvent): void {
  ...
  let tournamentGame = TournamentGame.load(event.params.contractAddress);
  if (tournamentGame !== null) {
    tournamentGame.ended = true;
    tournamentGame.save();
  }
...
export function handleTPlayerJoined(event: TPlayerJoinedEvent): void {
  ...
  let tournamentGame = TournamentGame.load(event.params.contractAddress);
  if (tournamentGame !== null) {
    tournamentGame.numOfPlayers = tournamentGame.numOfPlayers + 1;
    tournamentGame.playerAddresses = tournamentGame.playerAddresses.concat([
      event.params.player,
    ]);
    tournamentGame.save();
  }
